...
def patch_to(game_data: GameData, path: Path) -> None:
    ...
    if "data" in obj:  # 来自后端
        item_id_to_value: dict[str, float] = {
            x["itemId"]: x["itemValueAp"] for x in obj["data"]
        }
        item_id_to_name: dict[str, str] = {
            x["itemId"]: x["itemName"] for x in obj["data"]
        }
    ...
    not_in_yituliu_names = [
        item.name
        for item_id, item in game_data.items.items()
        if item_id not in item_id_to_value.keys()
    ]
    not_in_item_table_names = [
        item_name
