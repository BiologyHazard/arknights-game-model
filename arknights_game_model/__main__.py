def interactive():
    ...
    subprocess.run([sys.executable, "-i", "-m", "arknights_game_model", *sys.argv[1:]])
...
if __name__ == "__main__":
    ...
    from arknights_game_model.game_data import (
        CharacterDict,
        GameData,
        ItemDict,
        game_data,
    ...
    for character in c.values():
        globals()[character.id] = character
        globals()[character.name] = character
    for item in i.values():
        globals()[item.item_id] = item
        globals()[item.name] = item
    for uniequip in u.values():
        globals()[uniequip.uniequip_id] = uniequip
